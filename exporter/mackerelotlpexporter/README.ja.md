...
- `OTELCOL_MACKEREL_PREFER_IPV4`: 任意の非空文字列を設定すると、メトリクス用gRPCエンドポイントのホスト名をIPv4アドレスに名前解決してから接続します。DNSがAAAAレコードを返すもののIPv6の疎通がない環境でgRPC接続に失敗する場合に有用です。gRPCエンドポイントであるメトリックのエンドポイントにのみ影響します。
